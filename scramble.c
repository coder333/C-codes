...
int main(int argc, string argv[])
{
    ...
    while (true)
    {
        ...
        // Convert all letters input to uppercase
...
bool lookup(string s)
{
    ...
    // Check if the word has already been used to score 
    string in[100];
    static int i = 0;
    ...
    // Implement search algorithm to find word in dictionary
    int lp, mp, up;
    lp = 0; up = WORDS;
    mp = dictionary.size / 2;
    _Bool breakloop = 1;
    
        while (breakloop)
        {
            if (strcmp(dictionary.words[mp].letters, s) == 0)
            {
                breakloop = 0;
                return true;
            }
            else if (strcmp(dictionary.words[lp].letters, s) == 0)
            {
                breakloop = 0;
                return true;
            }
            else if (strcmp(dictionary.words[up].letters, s) == 0)
            {
                breakloop = 0;
                return true;
            }
            else if (strcmp(s, dictionary.words[mp].letters) < 0)
            {
                //lp = lp;
                up = mp;
                mp = mp / 2;
...
void scramble(void)
{
    ...
    // Use a two dimensional array temp[][] to hold the scrambled letters temporarily
    char temp[4][4];
    for (int i = 0; i < 4; i++)
    {
    ...
    // Store scrambled letters in grid[][]
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 4; j++)
        {
